extends Node2D
class_name EnemyDashAttackComponent

@onready var root_visual:Node2D
@onready var root_visual_texture:Texture2D
...
func _ready():
	...
	dash_length_vector = dash_length * Vector2.ONE

	timer.wait_time = dash_cooldown
	setup.call_deferred() # wait for owner instantiation

func setup():
	root_visual = owner.get_node("Visuals")
	if root_visual == null: push_error("Cannot Locate Visual Anchor Node from Owner")

	root_visual_texture = SpriteUtility2D.get_texture_from_visual(root_visual)
	if root_visual_texture == null: push_error("Found Visual Anchor, but Failed to Obtain Texture")

	phantom.texture = root_visual_texture
...
func sprite_tween_method(progression:float):
	owner.global_position = start_position.lerp(dash_end_position,progression)
...
func dash(direction:Vector2):
	...
	dash_length_vector = dash_length * direction
